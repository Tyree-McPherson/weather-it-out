<template>
  <v-container class="city-container">
    ...
    <v-row justify="center" align="center">
      <v-col>
        <v-card class="logo py-4 d-flex justify-center align-center flex-column">
          <h2>Temperature</h2>
  ...
  export default {
    ...
    data: () => ({
      ...
      async fetch() {
        ...
        const weather = await getCityWeatherData(this.$route.params.city, this.$config.serverlessDomain);
        this.weather = weather;
        this.timeZone = await getTimeZone(this.$route.params.city, this.$config.serverlessDomain);
        this.sunrise = getSunriseSunset(weather.city.sunrise, this.weather.city.timezone);
        this.sunset = getSunriseSunset(weather.city.sunset, this.weather.city.timezone);
    ...
    mounted() {
        document.title = `${capitalizeFirstLetter(this.$route.params.city)} - Weather It Out`;
    ...
    methods: {
      temperatureForecast(unit) {
        this.temperatureData = getTemperatureForecast(this.weather.list, this.oneDay, unit);
      ...
      changeForecast(forecast) {
        ...
        if (forecast === "one-day") {
          ...
          this.temperatureData = getTemperatureForecast(this.weather.list, true, this.celsius);
          this.precipitationData = getPrecipitationForecast(this.weather.list, true);
          ...
          this.temperatureData = getTemperatureForecast(this.weather.list, false, this.celsius);
          this.precipitationData = getPrecipitationForecast(this.weather.list, false);
